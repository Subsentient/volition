...
static int VLAPI_SpawnWorker(lua_State *const State);
...
static lua_State *InitScript(const Script::LuaJobType Type, const VLString &ScriptData, lua_State *const InState = nullptr);
//Globals
std::map<VLString, VLString> Script::LoadedScripts;
...
static const std::map<IntNameMapEnum, VLString> IntNameMap
{
	{ INTNAME_INT8, "int8" },
	{ INTNAME_UINT8, "uint8" },
...
static const std::map<Script::LuaJobType, VLString> JobTypeMap
{
	{ TOKEN_KEYVALPAIR(LUAJOB_INVALID) },
	{ TOKEN_KEYVALPAIR(LUAJOB_JOB) },
	{ TOKEN_KEYVALPAIR(LUAJOB_STARTUP) },
	{ TOKEN_KEYVALPAIR(LUAJOB_SNIPPET) },
	{ TOKEN_KEYVALPAIR(LUAJOB_WORKER) },
	{ TOKEN_KEYVALPAIR(LUAJOB_MAXVALUE) },
};
...
struct LuaWorker
{
	Conation::ConationQueue ToWorker, FromWorker;
	VLScopedPtr<VLThreads::Thread*> WorkerThread;
	VLString ScriptData, FunctionName;
	std::atomic_uint32_t RefCount;

	static int LuaStart(lua_State *State)
	{
		if (!VerifyLuaFuncArgs(State, { LUA_TTABLE }))
		{
			VLWARN("Bad arguments");
			lua_pushboolean(State, false);
			return 1;
		}
		
		lua_getfield(State, 1, "VL_LuaWorkerPtr");
		
		LuaWorker *This = static_cast<LuaWorker*>(lua_touserdata(State, -1));
		
		lua_getfield(State, 1, "VL_IsHostHandle");
		
		if (lua_type(State, -1) != LUA_TBOOLEAN)
		{
			VLWARN("Bad IsHost boolean!");
			lua_pushboolean(State, false);
			return 1;
		}
		
		const bool IsHost = lua_toboolean(State, -1);
		
		lua_pop(State, 1);
		
		if (!IsHost)
		{
			VLWARN("Attempted to start OURSELVES from the worker thread we're claiming to try and start!");
			lua_pushboolean(State, false);
			return 1;
		}
	...
	static int LuaIsAlive(lua_State *State)
	{
		if (!VerifyLuaFuncArgs(State, { LUA_TTABLE }))
		{
			VLWARN("Bad arguments");
		...
		if (!IsHost)
		{
			VLWARN("Worker asking if self is still alive. Returning true.");
			lua_pushboolean(State, true);
			return 1;
		}
		
		lua_pushboolean(State, This->WorkerThread->Alive());
		return 1;
	}
	
	static int LuaJoin(lua_State *State)
	{
		if (!VerifyLuaFuncArgs(State, { LUA_TTABLE }))
		{
			VLWARN("Bad arguments");
		...
		if (!IsHost)
		{
			VLWARN("Attempted to join from... the worker thread.... WHY?");
			lua_pushboolean(State, false);
			return 1;
		}
	...
	static int LuaRecv(lua_State *State)
	{
		if (!VerifyLuaFuncArgs(State, { LUA_TTABLE, LUA_TBOOLEAN }))
		{
			VLWARN("Bad arguments");
			return 0;
		}
		...
		const bool Blocking = lua_toboolean(State, 2);
		
		Conation::ConationQueue *Queue = IsHost ? &This->FromWorker : &This->ToWorker;
		
		Conation::ConationStream *RetVal = nullptr;
		
		if (Blocking)
		{
			RetVal = Queue->WaitPop();
		}
		else
		{
			RetVal = Queue->Pop();
		...
		if (!RetVal) return 0; //nil
		
		CloneConationStreamToLua(State, RetVal);
	...
	{
		if (!VerifyLuaFuncArgs(State, { LUA_TTABLE, LUA_TTABLE }))
		{
			VLWARN("Bad arguments");
			lua_pushboolean(State, false);
			return 1;
		...
		lua_pop(State, 1); //Probably unnecessary, but get rid of that bool before it causes trouble.

		//Get the ConationStream pointer in the table they just sent us.
		lua_getfield(State, 2, "VL_INTRNL");
		...
		Conation::ConationQueue *Queue = IsHost ? &This->ToWorker : &This->FromWorker;

		Queue->Push(new Conation::ConationStream{Stream->GetHeader(), Stream->GetArgData()}); //Copy, not move. This stream is using Lua's allocator, not ours.

		lua_pushboolean(State, true);
		return 1;
	}

	void NewHandle(lua_State *State, const bool IsHost)
	{
		//Store the methods and our LuaWorker pointer and whatnot in here.
		lua_newtable(State);

		lua_newtable(State); //metatable for the main table, since you can't set __gc directly on a lightuserdata.
		...
		lua_setmetatable(State, -2); //Set the metatable as the one for the table at the top of this function.

		lua_pushlightuserdata(State, this);
		lua_setfield(State, -2, "VL_LuaWorkerPtr");

		//Only the table at the top is left now.
		lua_pushboolean(State, IsHost);
		lua_setfield(State, -2, "VL_IsHostHandle");

		lua_pushcfunction(State, &LuaWorker::LuaSend);
		lua_setfield(State, -2, "Send");
		
		lua_pushcfunction(State, &LuaWorker::LuaRecv);
		lua_setfield(State, -2, "Recv");
		
		if (IsHost)
		{
			lua_pushcfunction(State, &LuaWorker::LuaJoin);
			lua_setfield(State, -2, "Join");
			
			lua_pushcfunction(State, &LuaWorker::LuaStart);
			lua_setfield(State, -2, "Start");
			
			lua_pushcfunction(State, &LuaWorker::LuaIsAlive);
			lua_setfield(State, -2, "Alive");
		}
		
		//Increment reference count now that we have a handle, and when this handle is destroyed, the __gc metamethod will decrement it.
		++this->RefCount;
	}

	static void *LaunchLua(LuaWorker *This)
	{
		VLScopedPtr<lua_State*, decltype(&lua_close)> State { luaL_newstate(), &lua_close };

		lua_State *const RetState = InitScript(LUAJOB_WORKER, This->ScriptData, State);

		if (!RetState)
		{
			VLWARN("Failed to start worker!");
			return nullptr;
		}

		//Find our function.
		lua_getglobal(State, This->FunctionName);

		if (lua_type(State, -1) != LUA_TFUNCTION)
		{
			VLWARN("Bad worker function provided!");
			return nullptr;
		}

		This->NewHandle(State, false);
		...
		if (lua_pcall(State, 1, 0, 0) != LUA_OK)
		{
			...
			VLWARN("Worker thread encountered a Lua error, got traceback: " + Traceback);
		}
		
		return nullptr;
	}

	static inline int LuaSpawnWorker(lua_State *State)
	{
		...
		const VLString FunctionName { lua_tostring(State, -1) };
		
		lua_pop(State, 1);
		
		lua_getglobal(State, "VL_LuaSource");
		
		if (lua_type(State, -1) != LUA_TSTRING)
		{
			VLWARN("No self-source set!");
			return 0;
		}
		
		const VLString ScriptData { lua_tostring(State, -1) };
		
		lua_pop(State, 1);
		
		LuaWorker *This = new LuaWorker(std::move(ScriptData), std::move(FunctionName));
		
		This->NewHandle(State, true); //Lua takes ownership of This from here on out.
		return 1;
	}
	
	LuaWorker(const VLString ScriptData, const VLString FunctionName)
		: ScriptData(std::move(ScriptData)), FunctionName(std::move(FunctionName)), RefCount(0)
	{
		this->WorkerThread = new VLThreads::Thread((VLThreads::Thread::EntryFunc)&LuaWorker::LaunchLua, this);
	}
};
...
static int VLAPI_SpawnWorker(lua_State *const State)
{
	return LuaWorker::LuaSpawnWorker(State);
}
...
static bool LoadVLAPI(lua_State *State)
{
	...
	for (const auto &Pair : VLAPIFuncs)
	{
		lua_pushcfunction(State, Pair.second);
		lua_setfield(State, -2, Pair.first);
	}
	...
	for (const auto &Pair : IntNameMap)
	{
		lua_pushinteger(State, Pair.first);
		lua_setfield(State, -2, Pair.second);
	}
	
	for (const auto &Pair : JobTypeMap)
	{
		lua_pushinteger(State, Pair.first);
		lua_setfield(State, -2, Pair.second);
	}
...
{ //Create a new instance of the class.
	...
	{ //Standard constructor.
		...
		if (Code < CMDCODE_INVALID || Code > CMDCODE_MAX)
		{ //Invalid value.
			lua_settop(State, 0);
			lua_pushnil(State);
			return 1;
...
static lua_State *InitScript(const Script::LuaJobType Type, const VLString &ScriptData, lua_State *const InState)
{
	VLDEBUG("Entered");
	...
	const bool Success = luaL_loadbuffer(State, ScriptData, ScriptData.Length(), "VLLUA") == LUA_OK;
	...
	//We store our own source for various purposes.
	lua_pushstring(State, ScriptData);
	lua_setglobal(State, "VL_LuaSource");
	
	//Set this BEFORE we call the initial script startup.
	lua_pushinteger(State, Type);
	lua_setglobal(State, "VL_LUAJOBTYPE");
...
bool Script::ExecuteScriptFunction(	const Script::LuaJobType Type,
...

{
	...
	lua_State *const RetState = InitScript(Type, ScriptData, State);
	...
	{ //Call the Lua function VL.ConationStream:New() to get a new copy for arguments. Pushes the Lua ConationStream onto the stack.
		throw ScriptError { FunctionName
...
{ //The job object we get is kinda butchered and limited compared to what we get via executing a script function
	...
	lua_State *const RetState = InitScript(LUAJOB_STARTUP, StartupScript, State);
